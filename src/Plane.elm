...
fromPoint (Point c p) = (c, p)

pointToPlane : Point a -> Plane a
pointToPlane (Point _ p) = p

pointToCoordinates : Point a -> Coordinates
pointToCoordinates (Point c _) = c

clampToCoordinates : X -> Y -> Plane a -> Coordinates
clampToCoordinates x y p = pointToCoordinates (clampToPoint x y p)
...
wrapToCoordinates x y p = pointToCoordinates (wrapToPoint x y p)
...
at : X -> Y -> Plane a -> Maybe a
at (X x) (Y y) (Plane p _ (Width w) _) =
    Array.get (x * w + y) p


clampAt : X -> Y -> Plane a -> a
clampAt x y p =
    get (clampToPoint x y p)


wrapAt : X -> Y -> Plane a -> a
wrapAt x y p =
    get (wrapToPoint x y p)
...
update f (Point (C c) (Plane p h w d)) = 
    case Array.get c p of 
        Nothing -> Plane p h w d
        Just a -> Plane (Array.set c (f a) p) h w d
...
height (Plane _ h _ _) = h
       
width : Plane a -> Width
width (Plane _ _ w _) = w